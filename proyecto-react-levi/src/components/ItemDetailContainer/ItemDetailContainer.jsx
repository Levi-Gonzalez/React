...
const ItemDetailContainer = () => {
    // invocamos el array con el product en singular porque va a ser un product.
      ...
      const { detalleId } = useParams () // ← dentro va ul obj {detailId: ''} //USEPARAMS: Es un hook de 'react-router-dom' capturando el parametro, → |
   ...
   // El UseEff.  se dispara cuándo se habia hecho el renderizado 
      useEffect(()=>{
        ...
        const queryProduct = doc(fireStore, 'productos', detalleId)
        getDoc(queryProduct)
        .then(resp => setProduct( { id: resp.id, ...resp.data() } ) )
        .catch( err => console.log(err) )
     }, [detalleId]) //  La dependencia, "[]" (que esta vacía) sirve para el montaje y que lo haga una sola vez después del montaje
